...
def build_point_table(archive_dir: Path):
    """返回 (tbl, X549)：tbl 含 X/Y/kind/split，X549 为 549×138 标准化特征。"""
...
def plot_run(
    ...
    sc1 = ax.scatter(
        ...
        cmap=CMAP, vmin=0, vmax=1, s=14, marker="o", edgecolors="0.4", linewidths=0.3,
        label=f"训练背景点 ({m_train_unl.sum()})",
    )
    ax.scatter(
        tbl.X[m_train_dep], tbl.Y[m_train_dep], c=point_prob[m_train_dep],
        cmap=CMAP, vmin=0, vmax=1, s=40, marker="*", edgecolors="k", linewidths=0.4,
    ...
    ax.set_title("(c) 训练点 + 测试矿点（点色=预测概率）")
    ax.set_xlabel("经度 (°E)"); ax.set_ylabel("纬度 (°N)")
    ax.legend(loc="lower right", fontsize=7)
    fig.colorbar(sc1, ax=ax, fraction=0.046, pad=0.04, label="预测概率")
    ...
    ax.scatter(tbl.X[m_train_unl], tbl.Y[m_train_unl], c=point_prob[m_train_unl],
               cmap=CMAP, vmin=0, vmax=1, s=14, marker="o", edgecolors="white",
    ...
    ax.scatter(tbl.X[m_train_dep], tbl.Y[m_train_dep], c=point_prob[m_train_dep],
               cmap=CMAP, vmin=0, vmax=1, s=42, marker="*", edgecolors="k",
    ...
    ax.set_title("(d) 训练点/测试矿点 + 全区预测概率")
    ax.set_xlabel("经度 (°E)"); ax.set_ylabel("纬度 (°N)")
    ax.legend(loc="lower right", fontsize=7)
    fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="预测概率")
