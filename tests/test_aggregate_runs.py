...
def _write_run(root: Path, name: str, *, seed: int, status: str, f1: float, mode: str = "train_from_archive_features"):
    directory = root / name
    ...
    (directory / "metrics.json").write_text(
        json.dumps({"mode": mode, "primary_metric": "f1", "f1": f1, "accuracy": f1 - 0.1}),
        encoding="utf-8",
    )
